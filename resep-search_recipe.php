...
// Ambil kata kunci pencarian, kategori, dan halaman dari POST
...
// Buat query untuk pencarian berdasarkan judul resep dengan join ke tabel user
if ($category === 'all') {
    $sql = "SELECT r.*, u.us_name FROM resep r LEFT JOIN user u ON r.us_id = u.us_id WHERE r.title LIKE '%$searchKeyword%' LIMIT $limit OFFSET $offset";
} else {
    $sql = "SELECT r.*, u.us_name FROM resep r LEFT JOIN user u ON r.us_id = u.us_id WHERE r.tipe = '$category' AND r.title LIKE '%$searchKeyword%' LIMIT $limit OFFSET $offset";
}

$result = $conn->query($sql);

// Tampilkan hasil pencarian dalam format HTML (kartu resep)
